...
def getVal(value,skipChecks=False):
    ...
    if value == "":
        errorHandle('VALUE ERROR: Missing value')
...
def execCode(code):
    ...
    while p < len(code):
        ...
        #for loop
            ...
            if not checkNext:
                for x in range(len(forLoopCode)):
                    if removeSpace(forLoopCode[x])[:5] == "NEXT ":
            ...
            forLoopCode = [x[4:] if removeSpace(x[:4])=="" else errorHandle("INTENDATION ERROR: Code executed in FOR loop should be indented inside loop by one tab space.") for x in forLoopCode]
