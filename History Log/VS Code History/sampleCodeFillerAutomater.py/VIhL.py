...
for fileCode  in files:
    ...
    if fileCode == "0.009 ioHandle.txt":
        result = subprocess.run(['python', 'decoder.py'], input="42", capture_output=True, text=True, check=True, encoding="utf-8")
    elif fileCode == "0.016 CaseStatement.txt":
        result = subprocess.run(['python', 'decoder.py'], input="W", capture_output=True, text=True, check=True, encoding="utf-8")
    elif fileCode == "0.018 RepeatLoop.txt":
        result = subprocess.run(['python', 'decoder.py'], input="Secret", capture_output=True, text=True, check=True, encoding="utf-8")
    elif fileCode == "0.022 FileHandling.txt":
        shutil.copy2("testFiles/fileA.txt","fileA.txt")
        shutil.copy2("testFiles/fileB.txt","fileB.txt")
        result = subprocess.run(['python', 'decoder.py'], capture_output=True, text=True, check=True, encoding="utf-8")
    else:
        result = subprocess.run(['python', 'decoder.py'], capture_output=True, text=True, check=True, encoding="utf-8")
    ...
    varList = json.loads(output[[True if x[0] == "{" else False for x in output].index(True)])
