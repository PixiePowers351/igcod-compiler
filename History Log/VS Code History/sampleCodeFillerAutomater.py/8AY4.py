...
for fileCode  in files:
    ...
    if fileCode == "0.009 ioHandle.txt":
        ...
        result = subprocess.run(['python', 'decoder.py'], input="W", capture_output=True, text=True, check=True)
    elif fileCode == "0.018 RepeatLoop.txt":
        result = subprocess.run(['python', 'decoder.py'], input="Secret", capture_output=True, text=True, check=True)
    ...
    item = {"name":fileCode,
            ...
            "stdout":stdout,
            "settings":[],
            "filesNeeded":["fileA.txt","fileB.txt"]}
