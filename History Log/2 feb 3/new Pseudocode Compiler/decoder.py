...
def getVal(value):
    ...
    elif "(" in list(value) and [x for x in value.split("(") if x != ""][0][-1].isalnum() and value == checkClosingBracket(value):
                ...
                for x in range(len(localVariables)):
                    ...
                    if not flag:
                        for y in range(len(functionCode)):
                            functionCode[y] = varReplace(functionCode[y],oldName,localVariables[x] if )
    ...
    #check if array
    elif "[" in list(value) and [x for x in value.split("[") if x != ""][0][-1].isalnum() and value == checkClosingBracket(value,t="arrayCheck"):
