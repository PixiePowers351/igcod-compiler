...
cd = open("C:/Users/pixie/Coding projects/new Pseudocode Compiler/code.txt","r", encoding="utf-8")
cd = cd.readlines()
...
#open settings
settings = open("C:/Users/pixie/Coding projects/new Pseudocode Compiler/settings.json","r", encoding="utf-8")
...
def varReplace(value,originalValue,replaceValue):
    ...
    while x <= len(value)-len(originalValue):
        ...
        if "".join(value[x:x+len(originalValue)]) == originalValue and (x == 0 or not value[x-1].isalpha()) and (x+1 > len(value)-len(originalValue) or not value[x+len(originalValue)].isalpha()):
...
def getVal(value):
    ...
    #functions and library routines
    elif value[firstSpecialChar(value)] == "(" and removeSpace(value)[-1] == ")":
        ...
        backupVar = copy.deepcopy(values)
    ...
    elif True in [x in value for x in '+-*/^<>=']:
    ...
    #split it to variables / values
        ...
        for z in [x for x in '+*/^'] + ["<>",">=","<=",">","<","="," AND "," OR "," NOT "]:
            valList = valList.replace(z,"$")
        
        valList = valList.split("$")

        #remove space and get value
        valList = [removeSpace(x) for x in valList]
        valList = [x for x in valList if x != "" and "-" not in list(x)]

        
        #sort valList by descending order of variable length
        flag = False
        while flag == False:
            flag = True
            for x in range(len(valList)-1):
                if len(valList[x]) < len(valList[x+1]):
                    buffer = valList[x]
                    valList[x] = valList[x+1]
                    valList[x+1] = buffer
                    flag = False
            
        valList = [[x, getVal(x)] for x in valList if not (x.isnumeric() or (value[0] == "-" and value[1:].isnumeric()))]
