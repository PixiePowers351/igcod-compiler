...
def bracketExcludedSplit(value,splitValue):
    ...
    if len(value.split(splitValue))>1:
        for x in range(len(value)-len(splitValue)+1):
            if value[x:x+len(splitValue)] == splitValue and inBracket == 0:
                temp.append(value[:x].replace("".join([x+splitValue for x in temp]),""))
            elif value[x+len(splitValue)-1] in ["(","["]:
...
def getVal(value):
    ...
    elif (True in [len(bracketExcludedSplit(value,m))>1 for m in list("+-*/^<>= ")+[f" {m} " for m in ["AND","OR","NOT"]]+[f"){m} " for m in ["AND","OR","NOT"]]+[f" {m}(" for m in ["AND","OR","NOT"]]+[f"){m}(" for m in ["AND","OR","NOT"]]+[f" {m}(" for m in ["AND","OR","NOT"]+["(NOT ","(NOT("]]] or value[:4]=="NOT " or value[:5]==" NOT("):
    ...
    #split it to variables / values
        ...
        for z in [" NOT "," AND "," OR "] + [x for x in '^/*+-'] + ["<>",">=","<=",">","<","="]+[f" {x} " for x in ["AND","OR","NOT"]]+[" "]:
            v = bracketExcludedSplit(valList,z)
            if len(v)>1:
                valList = "$".join(v)

        #exception for  "(NOT " and "NOT("
        if len(valList.split("(NOT ")) > 1:
            ""
