...
def checkClosingBracket(value,  t = "funcCheck"):
    ...
    for x in value:
        ...
        if not x.isalnum() and x not in delimiters and y == 0:
            s = ""
        else:
            s += x
...
def getVal(value):
    ...
    if (True in [len(bracketExcludedSplit(value,m))>1 for m in list("+-*/^<>= ")+[f" {m} " for m in ["AND","OR","NOT"]]+[f"){m} " for m in ["AND","OR","NOT"]]+[f" {m}(" for m in ["AND","OR","NOT"]]+[f"){m}(" for m in ["AND","OR","NOT"]]+[f" {m}(" for m in ["AND","OR","NOT"]+["(NOT ","(NOT("]]] or value[:4]=="NOT " or value[:5]==" NOT("):
    ...
    #split it to variables / values
        ...
        for z in [x for x in '^/*+-'] + ["<>",">=","<=",">","<","="]+[f" {x} " for x in ["AND","OR","NOT"]]+["(NOT "," "]:
        ...
        valList = [removeExtraBrackets(x) for x in valList]
        
        #remove the values which are not variables/functions
        #remove integers,strings, and empty vakues
        valList = [x for x in valList if not( x.isnumeric() or x == "") ]
        ...
        value = value.replace("^","**")
        value = value.replace("<>","!=")
        for x in [f"){x} " for x in ["AND", "OR","NOT"]]+[f" {x}(" for x in ["AND", "OR","NOT"]]+[f"){x}(" for x in ["AND", "OR", "NOT"]]+["(NOT "]+["(NOT ","(NOT("]+[f" {x} " for x in ["AND", "OR","NOT"]]:
        ...
        if concat and (valType.count("INTEGER") + valType.count("REAL") + valType.count("BOOLEAN") != len(valType) or valType.count("STRING") + valType.count("CHAR") != len(valType)):
    ...
    elif (value == checkClosingBracket(value) and value[-1] == ")" and not(True in [len(bracketExcludedSplit(value,m))>1 for m in list("+-*/^<>= ")+[f" {m} " for m in ["AND","OR","NOT"]]+[f"){m} " for m in ["AND","OR","NOT"]]+[f" {m}(" for m in ["AND","OR","NOT"]]+[f"){m}(" for m in ["AND","OR","NOT"]]+[f" {m}(" for m in ["AND","OR","NOT"]]] or value[:4]=="NOT " or value[:5]==" NOT(")):
    ...
    elif value[0] == '"' or value[0] == 'ꞌ' or value[0] == "'":
