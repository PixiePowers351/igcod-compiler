...
def getVal(value):
    ...
    elif value == checkClosingBracket(value) and value[-1] == ")":
            ...
            if mutableParameters:
                ...
                for y in range(len(functionCode)):
                    for x in range(len(parameters)):
                        ...
                        if not(False in [m.isalnum() for m in backupVar[x]]) or (backupVar[x] == checkClosingBracket(backupVar[x]) and backupVar[x][-1] == ")") or (backupVar[x] == checkClosingBracket(backupVar[x],t="arrayCheck") and backupVar[x][-1] == "]"):
                            functionCode[y] = varReplace(functionCode[y],parameters[x]['name'],backupVar[x])
                            localVariables[x] = ""
            
            #remove all iterated values
            localVariables = [x for x in localVariables if x != ""]
