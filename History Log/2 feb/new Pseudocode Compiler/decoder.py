...
def getVal(value):
    ...
    elif (value == checkClosingBracket(value) and value[-1] == ")" and not(True in [len(bracketExcludedSplit(value,m))>1 for m in list("+-*/^<>= ")+[f" {m} " for m in ["and","or","not"]]+[f"({m} " for m in ["and","or","not"]]+[f" {m})" for m in ["and","or","not"]]])):
            ...
            if mutableParameters:
                ...
                for y in range(len(functionCode)):
                    for x in range(len(parameters)):
                        ...
                        if not(False in [m.isalnum() for m in backupVar[x]]) or (backupVar[x] == checkClosingBracket(backupVar[x]) and backupVar[x][-1] == ")" and not(True in [len(bracketExcludedSplit(backupVar[x],m))>1 for m in list("+-*/^<>= ")+[f" {m} " for m in ["and","or","not"]]+[f"({m} " for m in ["and","or","not"]]+[f" {m})" for m in ["and","or","not"]]])) or (backupVar[x] == checkClosingBracket(backupVar[x],t="arrayCheck") and backupVar[x][-1] == "]"):
    ...
    #split it to variables / values
        ...
        for z in [" NOT "," AND "," OR "] + [x for x in '^/*+-'] + ["<>",">=","<=",">","<","=", " "]:
