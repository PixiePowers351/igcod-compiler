...
cd = cd.readlines()
operators = {"all":tuple(list("+-*/^<>=")+[f" {m} " for m in ["AND","OR","NOT"]]+[f"){m} " for m in ["AND","OR","NOT"]]+[f" {m}(" for m in ["AND","OR","NOT"]]+[f"){m}(" for m in ["AND","OR","NOT"]]+[f" {m}(" for m in ["AND","OR","NOT"]+["(NOT ","(NOT("]]),
        'bad':tuple([f"){x} " for x in ["AND", "OR","NOT"]]+[f" {x}(" for x in ["AND", "OR","NOT"]]+[f"){x}(" for x in ["AND", "OR", "NOT"]]+["(NOT("]),
        'good':tuple([f") {x} " for x in ["AND", "OR","NOT"]]+[f" {x} (" for x in ["AND", "OR","NOT"]]+[f") {x} (" for x in ["AND", "OR", "NOT"]]+["(NOT ("]),
        'andornotVar':tuple([f"){x} " for x in ["AND", "OR","NOT"]]+[f" {x}(" for x in ["AND", "OR","NOT"]]+[f"){x}(" for x in ["AND", "OR", "NOT"]]+["(NOT "]+["(NOT ","(NOT("]+[f" {x} " for x in ["AND", "OR","NOT"]]),
        'cmprs':tuple([x for x in '^/*+-'] + ["<>",">=","<=",">","<","="]+[f" {x} " for x in ["AND","OR","NOT"]])}

controlValues = tuple("AND OR NOT TRUE FALSE DECLARE BOOLEAN INTEGER REAL CHAR STRING ARRAY OF FOR NEXT INPUT OUTPUT CONSTANT DIV MOD LENGTH LCASE UCASE SUBSTRING ROUND RANDOM IF THEN ELSE ENDIF CASE OTHERWISE ENDCASE REPEAT UNTIL WHILE ENDWHILE PROCEDURE ENDPROCEDURE CALL FUNCTION ENDFUNCTION RETURN OPENFILE READ WRITE READFILE WRITEFILE CLOSEFILE".split(" "))
...
def errorHandle(errorText):
    ...
    error = True
    print(f"Line {codePointer+1}: {codeOg[codePointer]}")
    quit()
...
def getVal(value,skipChecks=False):
                ...
                if mutableParameters:
                    ...
                    for y in range(len(functionCode)):
                        for x in range(len(parameters)):
                            ...
                            if not(False in [m.isalnum() for m in backupVar[x]]) or (backupVar[x][-1] == ")" and backupVar[x] == checkClosingBracket(backupVar[x]) and not(True in backupVar[x][:4]=="NOT " or checkOperators(backupVar[x]))) or (backupVar[x] == checkClosingBracket(backupVar[x],t="arrayCheck") and backupVar[x][-1] == "]"):
    ...
    elif value in cache["evalCalls"].keys() or (value[:4]=="NOT " or checkOperators(value)):
    #operations (add/mult/sub/div)
    #split it to variables / values
        notConstant = True
        if cacheState and value in cache["evalCalls"].keys():
            comparision = cache["evalCalls"][value]["comparision"]
            concat = cache["evalCalls"][value]["concat"]
            ignoreTypeChecks = cache["evalCalls"][value]["ignoreTypeChecks"]
            valList = cache["evalCalls"][value]["valList"].copy()
            oldValList = cache["evalCalls"][value]["oldValList"].copy()
            ...
            if cacheState:
                cache["evalCalls"][value] = {"comparision":comparision,
                                             "concat":concat,
                                             "ignoreTypeChecks":ignoreTypeChecks,
                                             "valList":valList.copy(),
                                             "oldValList":oldValList.copy()}
        ...
        errorHandle(F"VALUE ERROR: {value} is not a variable or recognizable valid value for a constant. Perhaps you missed an apostrophe (\" or ')?")
...
def execCode(code):
    ...
    while p < len(code):
        ...
        if timerWarning != "" and time.time() - timer > int(timerWarning):
            estimatedTime = ((codePointer) / 10)*(len(codeOg)-codePointer)
            timerWarning = ""
            print("Code is slow.. please be patient..")
            print(f"Estimated time to complete: {estimatedTime}s")
        ...
        if code[p][:8] == "DECLARE ":
            #get variable name
            values = code[p][8:].split(":")

            if len(values) > 2 and "[" not in list(code[p]):
                errorHandle("SYNTAX ERROR: Invalid syntax for declaration\nCorrect form: DECLARE <Variable>:<TYPE>, replace <Variable> and <TYPE> with values you prefer")
            varName = removeSpace(values[0])

            if not varName.isalnum():
                errorHandle("VARIABLE ERROR: Variable name should only contain alphanumeric characters: A-Z, a-z, and 0-9")
            elif varName.isnumeric():
                errorHandle("VARIABLE ERROR: You cant declare a variable with only numbers in its name")
            elif varName in controlValues:
                errorHandle(f"VARIABLE ERROR: Cannot assign values to keywords such as {varName}")
            elif varName in var.keys():
                errorHandle("VARIABLE ERROR: Variable already declared")

            try:
                varType = removeSpace(values[1])
            except IndexError:
                errorHandle(f"SYNTAX ERROR: Make sure to write the type of variable during declaration as format: DECLARE {varName}:<TYPE>\nReplace <TYPE> with any type the variable should store, eg. INTEGER")
            ...
            #array special values
                ...
                commonErrorCases = {"INT":"INTEGER", "FLOAT":"REAL", "CHARACTER":"CHAR", "STR":"STRING", "BOOL":"BOOLEAN"}
                if varType in commonErrorCases.keys():
                    errorHandle(f"{varType} is not a valid data type, perhaps replace {varType} with {commonErrorCases[varType]}?")
                else:
        ...
        #declaring constants
            ...
            try:
                ...
                errorHandle("SYNTAX ERROR: Write a value for constant during declaration")
            ...
            if notConstant:
                errorHandle("VALUE ERROR: Cannot assign an expression, variable/constant to a constant")
        ...
        #for loop
            ...
            if not checkNext:
                for x in range(len(forLoopCode)):
                    if forLoopCode[x].strip()[:5] == "NEXT ":
                        codePointer = x
                        errorHandle("SYNTAX ERROR: Incorrect intendation for NEXT statement, it should be as the same gap as FOR statement")

                errorHandle("SYNTAX ERROR: NEXT statement missing, write in format NEXT <counter> where <counter> is the variable iterated in loop")

            forLoopCode = forLoopCode[:x]
            for x in range(len(forLoopCode)):
                if removeSpace(forLoopCode[x][:4])!="":
                    codePointer = x
                    errorHandle("INTENDATION ERROR: Code executed in FOR loop should be indented inside loop by one tab space.")
            ...
            if len(step) > 1:
                step[1] = removeSpace(removeExtraBrackets(step[1]))
                if step[1] == "":
                    errorHandle("SYNTAX ERROR: Missing step value, write in format ..STEP <step>")

                step = getVal(step[1],skipChecks = True)
            else:
                step = 1
            ...
            #get range to loop code
                ...
                errorHandle(f"SYNTAX ERROR: Incorrect syntax for FOR statement, should be\nFOR <counterVariable> {delimiter} <startCount> TO <endCount>")
        ...
        #calling procedures
            ...
            if not mutableParameters:
                ...
                for x in range(len(localVariables)):
                    if localVariables[x] not in deleteExcludedVariables:
        ...
        #error code if not comment
            ...
            #check if print error
            if code[p][:6] == "PRINT ":
                outputValue = code[p][6:]
                errorHandle(f"SYNTAX ERROR: Use OUTPUT {outputValue} instead of PRINT {outputValue}")
            
            #check for a comment format similar to other languages
            if code[p][0] == "#":
                errorHandle(f"SYNTAX ERROR: Comments in pseudocode are written as //{code[p][1:]}, not {code[p]}")

            #check for just A Randm entered vaue and ignore in that case
            if not getVal(code[p]):
